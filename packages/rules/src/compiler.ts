 ...
 */
function compileWebhookCall(baseUrl: string, ruleId: string, outcome: 'blocked' | 'notified' | 'allowed'): string {
 ...
 */
export function ruleToCode(baseUrl: string, rule: SpendRule): string {
  ...
  const lines = [
    `  // Rule: ${rule.label}`,
    `  if (${condExpr}) {`,
    compileWebhookCall(baseUrl, rule.id, isBlock ? 'blocked' : 'notified'),
    actionLines,
 ...
 */
export function compileRules(baseUrl: string, rules: SpendRule[]): string {
...
const beforeTransaction = (transaction) => {
  ...
  const ruleBodies = active.map((rule) => ruleToCode(baseUrl, rule)).join('\n\n');
